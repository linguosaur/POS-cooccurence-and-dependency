...
option = sys.argv[1]
posfile = codecs.open(sys.argv[2], 'r', 'utf-8')
for line in posfile:
	...
	if len(splitline) > 4:
		...
		if option == '-c': 
			...
			origPOSname = splitPOS[0]
		elif option == '-f': origPOSname = splitline[3]
		if origPOSname not in poslist: poslist.append(origPOSname)
		posIndex = poslist.index(origPOSname)
		print romaji + '\t' + repr(posIndex)
