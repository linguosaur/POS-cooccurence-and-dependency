...
for line in probsfile:
	...
	if splitline[0] != '' and splitline[0] != 'total:':
		pos1 = splitline[0]
		stats[pos1] = {}
	elif splitline[0] == '':
		stats[pos1][splitline[1]] = [string.atoi(splitline[2])] + map(string.atof, splitline[3:])
...
for line_i in range(len(depfilelines)):
	...
	for dep in splitdepline:
		try:
			...
			print dep
			print "len(splitposline):", len(splitposline)
			print "len(splitdepline):", len(splitdepline)
			print depfilelines[line_i]
...
for head in deps:
	...
	keys.reverse()
	for arg in keys:
		print '\t' + '\t'.join([arg, deps[head][arg], stats[head][arg][0], stats[head][arg][1], stats[head][arg][2], stats[arg][head][2]], stats[head][arg][3])
print
print 'nodeps'
print
for head in nodeps:
	...
	keys.reverse()
	for arg in keys:
		print '\t' + '\t'.join(arg, '--', stats[head][arg][0], stats[head][arg][1], stats[head][arg][2], stats[arg][head][2], stats[head][arg][3])
